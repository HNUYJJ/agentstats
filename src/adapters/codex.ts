...
import { baseName, linesOf, listFiles, looksLikeModel, mtimeMs, safeInt } from './util.js';
 ...
 * most recent `payload.model` seen while streaming the file. One session can
  ...
  async scan(home: string) {
    ...
    for (const dir of roots) {
      ...
      for (const file of batch) {
        ...
        let sessionId = baseName(file).replace(/\.jsonl$/, '').split('-').slice(-5).join('-') || baseName(file);
